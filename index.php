...
<body>
    ...
    <?php
        ...
        {
            $conta = new Conta(); //utilizando o método construtor para criar um objeto
            $conta->create($_GET["numero"], $_GET["agencia"], $_GET["proprietario"], 
                            $_GET["saldo"], $_GET["limite"]); //encapsular os valores no objeto
    ...
    <?php
            if (isset($_GET["depositar"])) //se clicou no botão depositar
            {
                $conta = new Conta();
                $conta->setSaldo($_GET["saldoAtual"]);
                echo $conta->depositar($_GET["valor"]);
            }
            else if (isset($_GET["sacar"])) //senão se clicou no botão sacar
            {
                $conta = new Conta();
                $conta->setSaldo($_GET["saldoAtual"]);
